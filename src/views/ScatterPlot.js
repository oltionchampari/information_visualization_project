...
export async function Scatter(songFeatureData, filter, columnSelection, selection, onSelectionChanged) {
    ...
    const plotDiv = await Plotly.newPlot("detail-view-2", data, layout, config);
    plotDiv.on("plotly_selected", function (eventData) {
        const selectedIds = eventData.points.map((p) => p.id);
        const selected = songFeatureData.filter((d) => d.song_id && selectedIds.includes(d.song_id));
