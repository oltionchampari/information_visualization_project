...
export function Heatmap() {
    ...
    const numColumns = 7;
    const numRows = 50;
    const data = Array.from({ length: numRows }, () => Array.from({ length: numColumns }, () => Math.random()));
    // Compute the correlations
    const correlations = data[0].map((_, i) => data[0].map((_, j) => i === j
        ? 1
        : ss.sampleCorrelation(data.map((row) => row[i]), data.map((row) => row[j]))));
