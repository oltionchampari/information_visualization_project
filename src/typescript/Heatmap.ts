...
let selectedCells = new Map<string, { x: string; y: string; value: number }>();
...
export function Heatmap(
  ...
  const rect = (detailView.node() as HTMLElement).getBoundingClientRect();
  ...
  const renderHeatmap = (width: number, height: number) => {
    ...
    const cells = svg
      ...
      .attr(
        "y",
        (d, i) => Math.floor(i / numColumns) * cellSize + 1 + padding.top
      )
      .attr("width", cellSize - 1)

      .attr("height", cellSize - 1)
      .attr("fill", (d) => color(d.value as any))
      .on("click", (event, d) => {
    ...
    const text = svg
      ...
      .attr(
        "x",
        (d, i) => (i % numColumns) * cellSize + 1 + padding.left + cellSize / 2
      )
      .attr(
        "y",
        (d, i) =>
          Math.floor(i / numColumns) * cellSize + 1 + padding.top + cellSize / 2
      )
    ...
    svg
      ...
      .attr(
        "y",
        Math.floor(corrValues.length + 1 / numColumns) * cellSize +
          padding.top +
          30
      )
      .style("font-size", "12px")
      ...
      .attr(
        "transform",
        (d, i) =>
          ` translate(${resizeMetric / 60 - 40}, 25) rotate(-45, ${
            i * cellSize + padding.left
          }, ${resizeMetric - padding.bottom + 20})`
      )
      .each(function (d, i) {
        var self = d3.select(this),
          text = self.text(),
  ...
  observer.observe(detailView.node() as HTMLElement);
}
